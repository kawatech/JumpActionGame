...
import com.badlogic.gdx.Preferences // ←追加する 11.1
import com.badlogic.gdx.ScreenAdapter
import com.badlogic.gdx.graphics.GL20
import com.badlogic.gdx.graphics.Texture
import com.badlogic.gdx.graphics.g2d.BitmapFont // ←追加する 10.1
...
class GameScreen(private val mGame: JumpActionGame) : ScreenAdapter() {
    companion object {
        ...
        val GUI_WIDTH = 320f    // ←追加する 9.2
        val GUI_HEIGHT = 480f   // ←追加する 9.2
    ...
    private val mGuiCamera: OrthographicCamera  // ←追加する 9.2
    private val mViewPort: FitViewport
    private val mGuiViewPort: FitViewport   // ←追加する 9.2
    ...
    private var mFont: BitmapFont   // ←追加する 10.1
    private var mScore: Int // ←追加する 10.1
    private var mHighScore: Int // ←追加する 10.1
    private var mPrefs: Preferences // ←追加する 11.1
    ...
    init {
        ...
        mFont = BitmapFont(Gdx.files.internal("font.fnt"), Gdx.files.internal("font.png"), false)   // ←追加する 10.1
        mFont.data.setScale(0.8f)   // ←追加する 10.1
        mScore = 0  // ←追加する 10.1
        mHighScore = 0  // ←追加する 10.1

        // ハイスコアをPreferencesから取得する
        mPrefs = Gdx.app.getPreferences("jp.techacademy.kawatech.jumpactiongame") // ←追加する 11.1
        mHighScore = mPrefs.getInteger("HIGHSCORE", 0) // ←追加する 11.1
    ...
    override fun render(delta: Float) {
        ...
        // スコア表示
        mGuiCamera.update() // ←追加する 10.1
        mGame.batch.projectionMatrix = mGuiCamera.combined  // ←追加する 10.1
        mGame.batch.begin() // ←追加する 10.1
        mFont.draw(mGame.batch, "HighScore: $mHighScore", 16f, GUI_HEIGHT - 15) // ←追加する 10.1
        mFont.draw(mGame.batch, "Score: $mScore", 16f, GUI_HEIGHT - 35)    // ←追加する 10.1
        mGame.batch.end()   // ←追加する 10.1
    }

    override fun resize(width: Int, height: Int) {
        mViewPort.update(width, height)
        mGuiViewPort.update(width, height)  // ←追加する 9.2
    }
    ...
    private fun updatePlaying(delta: Float) {
        ...
        if (Gdx.input.isTouched) {
            mGuiViewPort.unproject(mTouchPoint.set(Gdx.input.x.toFloat(), Gdx.input.y.toFloat(), 0f))   // ←修正する 9.2
            val left = Rectangle(0f, 0f, GUI_WIDTH / 2, GUI_HEIGHT) // ←修正する 9.2
            val right = Rectangle(GUI_WIDTH / 2, 0f, GUI_WIDTH / 2, GUI_HEIGHT)    // ←修正する 9.2
    ...
    private fun checkCollision() {
        ...
        for (i in 0 until mStars.size) {
            ...
            if (mPlayer.boundingRectangle.overlaps(star.boundingRectangle)) {
                star.get()
                mScore++    // ←追加する 10.1
                if (mScore > mHighScore) {  // ←追加する 10.1
                    mHighScore = mScore     // ←追加する 10.1

                    //ハイスコアをPreferenceに保存する
                    mPrefs.putInteger("HIGHSCORE", mHighScore)  // ←追加する 11.1
                    mPrefs.flush()  // ←追加する 11.1
    ...
    private fun updateGameOver() {
        if (Gdx.input.justTouched()) {                      // 追加 12.1
